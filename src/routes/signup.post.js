...
router.post("/hospital", async (req, res) => {
    ...
    const newHos = new hospitalSchema(hospital)
    try {
        await newHos.save()
...
router.post("/confirmedCase", async (req, res) => {
...
router.get("/confirmedCase", async (req, res) => {
...
router.patch("/confirmedCase/:id", async (req, res) => {
    let old_patient = req.params.id
    const patient = await confirmedCaseSchema.findById(old_patient)

    if (!patient) {
        return res.status(404).send({ "error": "patient does not exists" })
    }
    try {
        const updatedPatient = confirmedCaseSchema.findByIdAndUpdate(old_patient, {
